...
Because $$\ddot x_A = \ddot x_B$$, the non-sliding constraint is $$\ddot x + \ddot \theta h = 0$$.
...
In 3D formulation, the ZMP has to lie inside the convex hull of the contact points with the ground.
